...
describe('Generacion de Transferencias',() =>{
    it('Generacion de Transferencias Registradas', async()=>{
        const data = searchEntry(files.transferencias, [dataConditions.typeIs(dataTypes.transferencias),dataConditions.subtypeIs(dataSubtypes.Registradas), dataConditions.statusIs(dataStatus.pending)]);
